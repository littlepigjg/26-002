...
func (es *EventService) SetPanicGuard(fn store.PanicGuardFn) {
	es.panicGuard = fn
}

// ConsistencyCheck validates that the events map and the per-user index are
// consistent after concurrent ingestion. Returns a non-empty string describing
// the inconsistency if one is found, or an empty string if the state is clean.
func (es *EventService) ConsistencyCheck() string {
	ms, ok := es.store.(*store.MemoryStore)
	if !ok {
		return "store is not a *MemoryStore"
	}
	snapshot := ms.RawSnapshot()
	userIndex := ms.RawUserIndexSnapshot()

	expectedByUser := make(map[string]int)
	for _, evt := range snapshot {
		expectedByUser[evt.UserID]++
	}

	for uid, expected := range expectedByUser {
		actual := len(userIndex[uid])
		if actual != expected {
			return "mismatch for user " + uid
		}
	}

	for _, ids := range userIndex {
		for _, id := range ids {
			if _, ok := snapshot[id]; !ok {
				return "user index contains orphan id " + id
			}
		}
	}
...
func (es *EventService) CreateEvent(ctx context.Context, req *model.EventCreateRequest) (*model.Event, error) {
	...
	if es.panicGuard != nil && es.panicGuard(event.ID, event.PageURL) {
		panic("service-level panic guard triggered for event " + event.ID)
	}
