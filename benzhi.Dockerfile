# Use official Go image for build and runtime
...
# Build the application and install to PATH
RUN CGO_ENABLED=0 GOOS=linux go build -o /usr/local/bin/ubaas-server ./cmd/server/
...
# Default command - binary is in PATH, not affected by volume mount
